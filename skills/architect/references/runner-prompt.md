...
- Shared state: if two actors might both write, ask "what happens?" If the answer isn't "nothing," default to per-actor state with a merge at the read boundary, per [$principle-separate-before-serializing-shared-state](../../principle-separate-before-serializing-shared-state/SKILL.md).
...
- Encode invariants in types: hard-to-misuse types > runtime checks > prose comments, per [$principle-encode-lessons-in-structure](../../principle-encode-lessons-in-structure/SKILL.md).
- Validate at boundaries, trust types inside, per [$principle-boundary-discipline](../../principle-boundary-discipline/SKILL.md). Business logic as pure functions; the shell stays thin.
- Single source of truth per invariant. Derive instead of sync.
- Idempotent state transitions where applicable, per [$principle-make-operations-idempotent](../../principle-make-operations-idempotent/SKILL.md). Ask what happens if the operation runs twice or crashes halfway.
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy, per [$principle-laziness-protocol](../../principle-laziness-protocol/SKILL.md) and [$principle-minimize-reader-load](../../principle-minimize-reader-load/SKILL.md).
