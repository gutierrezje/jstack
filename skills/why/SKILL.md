...
3. Always inspect source control locally. Use the Why investigator route for read-only investigators covering distinct available external categories, using the matching file under `references/sources/` and `references/investigator-prompt.md`. Follow the route's default count and add a third only for an independent evidence lane.
